<?php
// blog.php - Category-specific blog page
include 'config.php';
...
try {
    // Redirect to homepage if category is invalid or missing
    if (!$category || !in_array($category, $valid_categories)) {
        echo "<script>window.location.href = 'index.php';</script>";
        exit;
    }

    $query = "SELECT * FROM posts WHERE category = ? ORDER BY publish_date DESC LIMIT 10";
...
<head>
    ...
    <title><?php echo htmlspecialchars($category); ?> Blog</title>
    <style>
        body {
...
<body>
    <header>
        <h1><?php echo htmlspecialchars($category); ?> Blog</h1>
    </header>
    <nav>
    ...
    <div class="posts">
        <?php if (empty($posts)): ?>
            <div class="no-posts">
                <p>No posts found in the <?php echo htmlspecialchars($category); ?> category.</p>
                <?php if (isLoggedIn()): ?>
